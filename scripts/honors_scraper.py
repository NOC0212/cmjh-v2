...
URLS = [
    "https://www.cmjh.tn.edu.tw/modules/tadnews/index.php?ncsn=10",
    "https://www.cmjh.tn.edu.tw/modules/tadnews/index.php?ncsn=10&g2p=2",
    "https://www.cmjh.tn.edu.tw/modules/tadnews/index.php?ncsn=10&g2p=3",
]

# 輸出位置（從 scripts/ 執行時指向專案根目錄 public/data）
...
def parse_honors(html, source_url):
    """解析 HTML 並回傳榮譽榜資料"""
...
def save_to_file(data, filepath):
    ...
    print(f"總共寫入 {len(data)} 筆榮譽榜到 {filepath}")
...
def main():
    ...
    for url in URLS:
        ...
        html = fetch_page(url)
        if html:
            page_data = parse_honors(html, url)
            all_honors.extend(page_data)

    if all_honors:
        save_to_file(all_honors, OUTPUT_FILE)
    else:
        print("沒有抓到任何榮譽榜資料。")
