...
import type { UpdateInfo } from "@/hooks/use-auto-update";
    ...
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent
        className={cn(
          ...
          "[&>button]:right-4 [&>button]:top-4 [&>button]:flex [&>button]:h-8 [&>button]:w-8 [&>button]:items-center [&>button]:justify-center [&>button]:rounded-full [&>button]:bg-black/20 [&>button]:text-white hover:[&>button]:bg-black/40"
        )}
      >
        ...
        <div className="p-8 sm:p-10">
          <div className="flex items-start gap-5 mb-8">
            <div className="shrink-0">
              <div className="relative w-20 h-20 rounded-2xl bg-brand-soft flex items-center justify-center shadow-sm overflow-hidden">
                <img src="/favicon.png" alt="App Icon" className="w-14 h-14" />
                <motion.div
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  transition={{ type: "spring", bounce: 0.4, delay: 0.3 }}
                  className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-success shadow-lg"
                >
            ...
            <div className="min-w-0 flex-1 pt-1.5">
              <h2 className="text-2xl font-bold tracking-tight text-foreground truncate">崇明國中v2</h2>
              ...
              <motion.div
                ...
                className="flex items-center gap-1.5 mt-2 text-[11px] text-success font-medium"
              >
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2 }}>
            <Button className="h-14 w-full gap-2.5 text-lg font-bold rounded-xl shadow-lg shadow-primary/20 transition-transform hover:scale-[1.01] active:scale-[0.98]" onClick={onClose}>
