...
export function useCalendarEvents() {
  ...
  const addEvent = (date: string, title: string) => {
    setCustomEvents((prev) => [
      ...prev,
      { id: `event-${Date.now()}`, date, title, isCustom: true },
    ]);
  };
  ...
  /** 將自訂事件按月份分組 */
  const getCustomEventsByMonth = (): Record<string, CustomCalendarEvent[]> => {
    const grouped: Record<string, CustomCalendarEvent[]> = {};
