...
export const Home = ({ onComplete }: HomeProps) => {
    ...
    return (
        <main ref={mainRef} className="fixed inset-0 w-full h-full bg-white dark:bg-slate-950 transition-colors duration-500 overflow-y-auto pt-10">
            ...
            {/* 內容置中容器 */}
            <div className="relative z-10 min-h-screen w-full flex flex-col items-center justify-start pt-16 md:pt-24 p-4 sm:p-6 md:p-12">
                <AnimatePresence mode="wait">
                    ...
                    >
                        {/* 文字內容 */}
                        <div className="space-y-6 sm:space-y-8 animate-slide-in-left max-w-2xl">
                            <div>
                                <span className="inline-flex items-center gap-2 px-3 sm:px-4 py-1 rounded-full bg-cyan-500/10 text-cyan-700 dark:text-cyan-400 text-xs sm:text-sm font-bold border-2 border-cyan-600/30 dark:border-cyan-500/30 mb-4 sm:mb-6 uppercase tracking-wider transition-colors">
                                    <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                                    v1.6.0 正式推出
                                    <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                                </span>
                                <h1 className="text-[2.5rem] sm:text-5xl md:text-7xl font-['Noto_Sans_TC'] font-semibold tracking-tighter leading-none mb-4 sm:mb-6 flex flex-col drop-shadow-sm break-keep">
                                ...
                                <p className="text-base sm:text-xl text-slate-600 dark:text-slate-400 font-medium max-w-lg mx-auto lg:mx-0 leading-relaxed transition-colors">
                                    重新定義網站，結合現代美學與智能工具。
                            ...
                            <div className="flex flex-wrap justify-center lg:justify-start gap-3 sm:gap-4 pt-4">
                                <Button
                                    size="lg"
                                    className="h-14 sm:h-16 px-8 sm:px-12 text-lg sm:text-xl font-bold rounded-2xl text-white bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-500 hover:to-cyan-500 hover:scale-[1.02] active:scale-95 transition-all gap-3 overflow-hidden relative group shadow-[0_0_40px_rgba(13,148,136,0.3)]"
                                ...
                                <Button
                                    variant="outline"
                                    size="lg"
                                    className="h-14 sm:h-16 px-8 sm:px-10 text-lg sm:text-xl font-bold rounded-2xl border-2 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-white bg-transparent hover:bg-slate-100 dark:hover:bg-slate-800 hover:scale-[1.02] active:scale-95 hover:border-slate-400 dark:hover:border-slate-600 transition-all overflow-hidden relative group"
                                    onClick={() => window.open('https://github.com/NOC0212/cmjh-v2', '_blank')}
                                >
                                    <Github className="w-5 h-5 sm:w-6 sm:h-6 group-hover:rotate-6 transition-transform" />
                                    GitHub 原始碼
                                </Button>
                            </div>
                        </div>
                        ...
                        {/* 圖形側 - 手機圖片 */}
                        <div className="relative w-[280px] sm:w-80 h-[380px] sm:h-[500px] md:w-[500px] md:h-[650px] animate-slide-in-right flex-shrink-0 flex items-center justify-center">
                            {/* 手機後方的環境光圈 */}
                            <motion.div
            ...
            {/* 功能亮點 Bento Grid */}
            <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-32 min-h-screen flex flex-col justify-center space-y-10 sm:space-y-16">
                <div className="text-center lg:text-left space-y-4">
                    ...
                    <h2 className="text-4xl sm:text-5xl md:text-7xl font-black tracking-tighter leading-tight text-slate-900 dark:text-white transition-colors">
                        為你打造的<br />
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 sm:gap-6">
                    ...
                    <motion.div 
                        whileHover={{ y: -5 }}
                        className="md:col-span-8 md:row-span-2 p-5 sm:p-8 rounded-2xl sm:rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-[0_20px_60px_rgba(0,0,0,0.04)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.3)] flex flex-col justify-between group transition-all duration-500 relative overflow-hidden"
                    >
                        {/* 光暈 - 左上方向 */}
                        ...
                        <div className="space-y-4 relative z-10">
                            <div className="w-12 sm:w-14 h-12 sm:h-14 rounded-2xl bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 group-hover:bg-teal-500/10 group-hover:dark:bg-teal-500/20 group-hover:text-teal-600 group-hover:dark:text-teal-400 transition-all duration-500">
                                <Megaphone className="w-6 sm:w-7 h-6 sm:h-7" />
                            </div>
                            <div className="space-y-2">
                                <h3 className="text-2xl sm:text-3xl font-black tracking-tighter text-slate-900 dark:text-white">最新公告</h3>
                    ...
                    <motion.div 
                        whileHover={{ y: -5 }}
                        className="md:col-span-4 p-5 sm:p-8 rounded-2xl sm:rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-[0_20px_60px_rgba(0,0,0,0.04)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.3)] flex flex-col gap-6 transition-all duration-500 relative overflow-hidden group"
                    >
                        {/* 光暈 - 右上方向 */}
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none rounded-[2.5rem]">
                    ...
                    <motion.div 
                        whileHover={{ y: -5 }}
                        className="md:col-span-4 p-5 sm:p-8 rounded-2xl sm:rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-[0_20px_60px_rgba(0,0,0,0.04)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.3)] flex flex-col justify-between transition-all duration-500 relative overflow-hidden group"
                    >
                        {/* 光暈 - 左下方向 */}
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none rounded-[2.5rem]">
                    ...
                    <motion.div 
                        whileHover={{ y: -5 }}
                        className="md:col-span-5 p-5 sm:p-8 rounded-2xl sm:rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-[0_20px_60px_rgba(0,0,0,0.04)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.3)] space-y-4 transition-all duration-500 relative overflow-hidden group"
                    >
                        {/* 光暈 - 右下方向 */}
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none rounded-[2.5rem]">
                        ...
                        <Utensils className="w-6 sm:w-7 h-6 sm:h-7 text-slate-400 group-hover:text-emerald-500 transition-all duration-500 relative z-10" />
                        <div className="space-y-2 relative z-10">
                            <h4 className="text-lg sm:text-xl font-black tracking-tighter text-slate-900 dark:text-white">午餐菜單</h4>
                    ...
                    <motion.div 
                        whileHover={{ y: -5 }}
                        className="md:col-span-3 p-5 sm:p-8 rounded-2xl sm:rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-[0_20px_60px_rgba(0,0,0,0.04)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.3)] space-y-4 transition-all duration-500 relative overflow-hidden group"
                    >
                        {/* 光暈 - 左中方向 */}
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none rounded-[2.5rem]">
                    ...
                    <motion.div 
                        whileHover={{ y: -5 }}
                        className="md:col-span-4 p-5 sm:p-8 rounded-2xl sm:rounded-[2.5rem] bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-[0_20px_60px_rgba(0,0,0,0.04)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.3)] space-y-4 transition-all duration-500 relative overflow-hidden group"
                    >
                        {/* 光暈 - 右中方向 */}
                        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none rounded-[2.5rem]">
            ...
            {/* 開源區塊 */}
            <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-32 min-h-screen flex flex-col justify-center">
                <motion.div 
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    className="p-6 sm:p-12 md:p-16 rounded-2xl sm:rounded-[3rem] bg-gradient-to-br from-white to-slate-50/50 dark:from-slate-900 dark:to-slate-900/50 border border-slate-100 dark:border-slate-800 shadow-[0_40px_100px_rgba(0,0,0,0.03)] dark:shadow-[0_40px_100px_rgba(0,0,0,0.4)] relative overflow-hidden"
                >
                    <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                        <div className="space-y-10">
                            ...
                            <div className="flex flex-wrap gap-3 sm:gap-4">
                                <Button className="h-12 sm:h-14 px-6 sm:px-8 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold gap-3 text-base sm:text-lg shadow-xl shadow-slate-200 dark:shadow-none transition-all hover:scale-105 active:scale-95" onClick={() => window.open('https://github.com/NOC0212/cmjh-v2', '_blank')}>
                                    <Github className="w-5 h-5 sm:w-6 sm:h-6" /> 檢視原始碼
                                </Button>
                                <Button variant="outline" className="h-12 sm:h-14 px-6 sm:px-8 rounded-2xl border-2 border-slate-200 dark:border-slate-800 bg-white dark:bg-transparent font-bold gap-3 text-base sm:text-lg transition-all hover:bg-slate-50 dark:hover:bg-slate-800 hover:scale-105 active:scale-95 shadow-sm text-slate-900 dark:text-white" onClick={() => window.open('https://github.com/NOC0212/cmjh-v2/issues', '_blank')}>
                                    <MessageSquare className="w-5 h-5 sm:w-6 sm:h-6" /> 提出建議
                                </Button>
                            </div>

                            <div className="grid grid-cols-3 gap-6 sm:gap-8 pt-6 border-t border-slate-100 dark:border-slate-800">
                                <div>
                                    <p className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mb-1 tracking-tighter">100+</p>
