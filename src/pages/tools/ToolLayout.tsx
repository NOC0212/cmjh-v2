...
export function ToolLayout({ children, title, onBack }: ToolLayoutProps) {
  ...
  return (
    <div className="h-screen w-full flex flex-col overflow-hidden bg-background">
      <header className="flex-none sticky top-0 z-40 bg-background/80 backdrop-blur-xl">
        <div className="flex h-14 items-center gap-1.5 px-2 lg:px-4 border-b border-border/60">
          <Button
            variant="ghost"
            size="icon"
            onClick={handleBack}
          ...
          <h1 className="text-sm sm:text-base font-bold text-brand-gradient shrink-0 mr-2">{title}</h1>
        ...
        <div className={cn("md:hidden overflow-hidden transition-all duration-300 ease-in-out", menuOpen ? "max-h-96 border-t border-border/40" : "max-h-0")}>
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-4 lg:p-8 flex flex-col">
        <div className="max-w-7xl w-full mx-auto flex-1 flex flex-col lg:justify-center">{children}</div>

        <footer className="mt-12 border-t border-border/60 py-6 px-4 lg:px-6 rounded-t-3xl">
