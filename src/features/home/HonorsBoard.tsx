...
import { useHonors } from "@/hooks/use-static-data";
import type { HonorItem } from "@/lib/data";
...
export function HonorsBoard() {
  ...
  // 使用 use-static-data 統一管理榮譽榜資料
  const {
    data: honors = [],
    isLoading: loading,
  } = useHonors();

  // 清理收藏中的無效 ID（資料載入完成後執行）
  useEffect(() => {
    if (honors.length > 0) {
      const validIds = honors.map((h: HonorItem) => `honor-${h.date}-${h.title}`);
      cleanupFavorites("honor", validIds);
    }
  }, [honors, cleanupFavorites]);

  const totalPages = Math.max(1, Math.ceil(honors.length / ITEMS_PER_PAGE));
    ...
    <section id="honors">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-foreground">榮譽榜</h2>
        </div>
