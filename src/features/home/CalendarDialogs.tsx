...
import { Settings, Trash2, RotateCcw, Edit, Plus } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { useCalendarEvents, type CustomCalendarEvent } from "@/hooks/use-calendar-events";
...
/** 將可用月份按年份分組 */
function useGroupedMonths(availableMonths: string[]) {
  return useMemo(() => {
...
/** 可用月份（props 未提供時自行 fetch） */
function useAvailableMonths(
  initialMonths: string[] | undefined,
  ...
  }, [initialMonths, setAvailableMonths]);
}
...
export function CalendarAddDialog({ availableMonths: initialMonths }: { availableMonths?: string[] }) {
  ...
  useAvailableMonths(initialMonths, setAvailableMonths);
  ...
  const groupedMonths = useGroupedMonths(availableMonths);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[95vw] max-w-md rounded-2xl bg-card border-border shadow-lg p-0">
        ...
        <div className="px-6 py-4 space-y-5">
              ...
              >
                ...
                <SelectContent>
                  {Object.keys(groupedMonths).sort().map((year) => (
                    <div key={year}>
                      <div className="px-2 py-1.5 text-[10px] font-bold uppercase text-muted-foreground/60 bg-muted/30">
                        {year}年
                      </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              className="h-10 rounded-xl border-border/40 focus:border-primary/50"
        ...
        <DialogFooter className="p-6 pt-2">
          <Button onClick={handleSave} className="w-full h-11 rounded-xl font-bold shadow-sm">
            儲存事件
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="w-[95vw] max-w-md h-[85vh] md:h-auto md:max-h-[85vh] flex flex-col overflow-hidden rounded-2xl bg-card border-border shadow-lg p-0">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
            {/* 編輯表單（僅在編輯時顯示） */}
            {editingId && (
              <div className="space-y-4 p-5 bg-primary/5 rounded-xl border border-primary/10 shadow-sm">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-bold text-primary">編輯事件內容</h3>
                  <Button
                    variant="ghost"
                    size="sm"
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between px-2">
                <h3 className="text-xs font-bold tracking-widest uppercase text-muted-foreground">已安排的事件</h3>
              </div>
              <div className="flex flex-col gap-2.5">
                {sortedEvents.length > 0 ? (
                  sortedEvents.map((event) => (
                    <div
                      key={event.id}
                      className="flex items-center gap-3 bg-background rounded-xl p-4 border border-border/50 shadow-sm hover:shadow-md transition-all group"
                    >
          ...
          <DialogFooter className="p-6 bg-muted/5 border-t border-border/10 shrink-0">
            <div className="flex items-center justify-between w-full gap-3">
              ...
              <Button onClick={() => setOpen(false)} className="h-11 rounded-xl font-bold shadow-sm px-8">完成管理</Button>
            </div>
          </DialogFooter>
        </DialogContent>
      </Dialog>
