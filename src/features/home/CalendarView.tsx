...
import { useSchoolCalendar } from "@/hooks/use-static-data";
import { cn } from "@/lib/utils";
import { CalendarAddDialog, CalendarDialog } from "./CalendarDialogs";
...
type CalendarDataMap = Record<string, CalendarEventItem[]>;
...
/** 取得初始月份 */
function getInitialMonth(data: CalendarDataMap | undefined): string {
  if (!data) return "";
...
export function CalendarView() {
  ...
  // 校曆資料（useSchoolCalendar 統一管理）
  const {
    data: rawCalendarData,
    isLoading: loading,
  } = useSchoolCalendar();

  // 正規化校曆資料（加入 isCustom 標記）
  const calendarData = useMemo<CalendarDataMap>(() => {
    if (!rawCalendarData) return {};
    const normalized: CalendarDataMap = {};
  ...
  const mergedCalendarData = useMemo(() => {
    const merged: CalendarDataMap = { ...calendarData };
  ...
  const eventsByDay = useMemo(() => {
    ...
    return mergedCalendarData[selectedMonth].reduce<Record<number, CalendarEventItem[]>>((acc, event) => {
  ...
  const renderEventPopover = (day: number, dayEvents: CalendarEventItem[]) => {
                ...
                <div className="flex gap-1 justify-center mt-0.5 md:mt-1">
                  {dayEvents.slice(0, 3).map((event, index) => (
                    <span
                      ...
                      className={cn(
                        "rounded-full h-1.5 w-1.5 md:h-2 md:w-2",
                        event.isCustom ? "bg-primary/40" : "bg-primary"
                      )}
                    />
                  ))}
                </div>
    ...
    <section id="calendar">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-foreground">行事曆</h2>
